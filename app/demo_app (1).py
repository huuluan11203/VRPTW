...
st.title('VRPTW Demo – Chọn điểm, chỉnh sửa & chạy solver 🚚')
...
# =========================
st.markdown("### 1) Bản đồ – Click để thêm khách hàng")
...
# 2) BẢNG CHỈNH SỬA
# =========================
st.markdown("### 2) Danh sách điểm đã chọn")
...
# =========================
st.markdown("### 3) Lưu danh sách & Chạy VRPTW")
save_col, run_col = st.columns([1,1])
with save_col:
    save_csv = st.button("💾 Lưu danh sách (CSV)", type="primary", use_container_width=True)
with run_col:
    do_run = st.button("▶️ Chạy VRPTW", use_container_width=True)
...
# =============================
# Section: 4) Kết quả tuyến & thống kê
# =============================
st.markdown("### 4) Kết quả tuyến & thống kê")

# Chỉ hiện sau khi bấm "Chạy VRPTW"
if not st.session_state.get("show_result"):
    st.info("Nhấn **▶️ Chạy VRPTW** để xem bản đồ & bảng kết quả.")
else:
    # Tải dữ liệu
    ...
    if customers.empty:
        ...
        try:
            ...
            # Tính chữ ký cache & khoảng cách thực tế (OSRM)
            ...
            def _build_result_map(route_data, depot_coord_show):
                ...
                for d in route_data:
                    ...
                    if d.get("geometry"):
                        ...
                        folium.PolyLine(
                            d["coordinates"], color=color, weight=3, opacity=0.9,
                            popup=f"Route #{d['route_idx']} — {d.get('final_distance_km', 0.0):.2f} km"
                        ).add_to(mm)
                    ...
                    for seq, (lat, lon) in enumerate(coords[1:-1], start=1):
                        ridx = seq - 1
                        cust_id = d["route_ids"][ridx] if 0 <= ridx < len(d["route_ids"]) else ''
                        folium.CircleMarker(
                            [lat, lon], radius=6, color=color, fill=True, fillOpacity=0.7,
                            popup=f"Route {d['route_idx']} - Điểm {seq} - ID {cust_id}",
                        ).add_to(mm)

                return mm

            # Cache bản đồ theo chữ ký ổn định
            result_sig = (routes_sig, cust_sig, depot_sig)
            if st.session_state.get("result_sig") != result_sig:
                st.session_state["result_map"] = _build_result_map(route_data, depot_coord_show)
                st.session_state["result_sig"] = result_sig

            _ = st_folium(st.session_state["result_map"], height=620, use_container_width=True, key="map_result")
st.markdown("---")
